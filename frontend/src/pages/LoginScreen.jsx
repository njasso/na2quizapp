// src/pages/LoginPage.jsx — NA2Quiz Professional Design
...
const LoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('userInfo', JSON.stringify({ email: data.email, username: data.username, role: data.role, name: data.name, _id: data._id, token: data.token }));
      navigate('/evaluate');
    } catch (err) {
      let msg = 'Erreur de connexion.';
      if (axios.isAxiosError(err)) {
        if (err.code === 'ECONNABORTED') msg = 'Délai de connexion dépassé.';
        else if (err.response?.status === 401) msg = 'Email ou mot de passe incorrect.';
        else if (err.response?.status === 404) msg = 'Endpoint introuvable — vérifiez la config API.';
        else if (err.response?.status === 500) msg = 'Erreur serveur — réessayez plus tard.';
        else if (err.message?.includes('Network Error')) msg = 'Problème de réseau. Vérifiez votre connexion.';
    ...
    <div style={{
      ...
      >
        ...
        <div style={{ textAlign: 'center', marginBottom: '36px' }}>
          ...
          >
            <span style={{ fontFamily: "'Sora', sans-serif", fontWeight: 800, fontSize: '1.25rem', color: '#fff' }}>N²</span>
          </motion.div>
          <h1 style={{
          ...
          <form onSubmit={handleLogin} noValidate>
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              <div style={{ textAlign: 'right', marginTop: '7px' }}>
                <a href="/forgot-password" style={{ fontSize: '0.8rem', color: '#60a5fa', textDecoration: 'none' }}>
                  Mot de passe oublié ?
                </a>
              </div>
            </div>
            ...
            >
              {isLoading ? (
                <><Loader2 size={18} style={{ animation: 'spin 1s linear infinite' }} /> Connexion en cours…</>
              ) : (
