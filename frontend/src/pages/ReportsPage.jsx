// src/pages/ReportsPage.jsx — Refonte complète
// Architecture : 4 onglets | Pagination | Modal bulletin | Groupement par épreuve
...
import {
  ...
  TrendingUp, Award, BookOpen, Link
} from 'lucide-react';
        ...
        >
          ...
          <div style={{ padding: '18px 24px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <Btn onClick={printBulletin} color="#3b82f6">
          ...
          <div style={{ overflowY: 'auto', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {[
                ...
                ['Épreuve', [
                  ['Matière',  report.subject || report.examId?.subject || '—'],
                  ['Domaine',  report.domain  || report.examId?.domain  || '—'],
            ...
            {qs.length > 0 && (
              <div>
                <div style={{ color: '#64748b', fontSize: '0.7rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>
                  Détail des réponses
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {qs.map((q, i) => {
                    const qId      = q._id?.toString?.() || String(i);
                    const student  = ans[qId] ?? null;
                    const correct  = q.correctAnswer || q.answer || '';
                    const isOk     = student && String(student).trim() === String(correct).trim();
                    return (
                      <div key={i} style={{ padding: '10px 12px', borderRadius: 9, border: `1px solid ${isOk ? 'rgba(16,185,129,0.3)' : 'rgba(239,68,68,0.3)'}`, background: isOk ? 'rgba(16,185,129,0.05)' : 'rgba(239,68,68,0.05)' }}>
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 4 }}>
                          {isOk ? <CheckCircle size={13} color="#10b981" style={{ flexShrink: 0, marginTop: 2 }}/> : <XCircle size={13} color="#ef4444" style={{ flexShrink: 0, marginTop: 2 }}/>}
                          <span style={{ color: '#e2e8f0', fontSize: '0.83rem', fontWeight: 500, lineHeight: 1.4 }}>
                            Q{i+1}. {q.question || q.text || '—'}
                          </span>
                        </div>
                        <div style={{ paddingLeft: 21, fontSize: '0.78rem' }}>
                          <span style={{ color: '#64748b' }}>Réponse : </span>
                          <span style={{ color: isOk ? '#10b981' : '#ef4444', fontWeight: 600 }}>{student || '—'}</span>
                          {!isOk && <><span style={{ color: '#334155' }}> | Correcte : </span><span style={{ color: '#10b981', fontWeight: 600 }}>{correct}</span></>}
                        </div>
                      </div>
                    );
                  })}
            ...
            <div style={{ padding: '10px 14px', background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              ...
              <button
                onClick={() => { navigator.clipboard?.writeText(`${NODE_BACKEND_URL}/api/bulletin/${report._id}`); toast.success('Lien copié !'); }}
                style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 5, padding: '5px 10px', borderRadius: 7, background: 'rgba(99,102,241,0.12)', border: '1px solid rgba(99,102,241,0.3)', color: '#a5b4fc', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 600 }}>
                <Copy size={12}/> Copier
              </button>
            </div>
          </div>
...
const ReportsPage = () => {
  ...
  const [tab, setTab]           = useState('results'); // results | sessions | rankings | stats
  const [search, setSearch]     = useState('');
  const [filterExam, setFilterExam] = useState('');
  const [filterStatus, setFilterStatus] = useState(''); // '' | 'passed' | 'failed'
  const [page, setPage]         = useState(1);
  const [selectedReport, setSelectedReport] = useState(null); // modal
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const r = Array.isArray(resResp.data) ? resResp.data
        : Array.isArray(resResp.data?.data) ? resResp.data.data : [];
      const e = Array.isArray(examResp.data) ? examResp.data
        : Array.isArray(examResp.data?.data) ? examResp.data.data : [];
      setResults(r);
      setExams(e);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    setRankLoading(true);
    axios.get(`${NODE_BACKEND_URL}/api/rankings/${rankExamId}`)
  ...
  const printRankingPDF = useCallback(() => {
    ...
    const exam = exams.find(e => e._id === rankExamId);
    const examTitle = exam?.title || 'Classement';
    const medals = ['🥇', '🥈', '🥉'];
    ...
    const highest = Math.max(...rankings.map(r => r.percentage || 0));
    ...
    <head>
      <meta charset="UTF-8">
      <title>Classement - ${escapeHtml(examTitle)}</title>
      <style>
    ...
    toast((t) => (
      <div style={{ background: '#1e293b', padding: 16, borderRadius: 12, color: '#fff' }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          <button onClick={() => { toast.dismiss(t.id); axios.delete(`${NODE_BACKEND_URL}/api/results/${id}`).then(() => { setResults(prev => prev.filter(r => r._id !== id)); toast.success('Supprimé'); }).catch(() => toast.error('Erreur')); }}
  ...
  }, [results, search, filterExam, filterStatus]);
  ...
  useEffect(() => { setPage(1); }, [search, filterExam, filterStatus]);
            ...
            <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <select value={filterExam} onChange={e => setFilterExam(e.target.value)}
                  style={{ padding: '8px 12px', background: '#0d1424', border: '1px solid rgba(59,130,246,0.15)', borderRadius: 9, color: filterExam ? '#e2e8f0' : '#475569', fontSize: '0.82rem', outline: 'none', maxWidth: 240 }}>
                ...
                {(search || filterExam || filterStatus) && (
                  <Btn onClick={() => { setSearch(''); setFilterExam(''); setFilterStatus(''); }} color="#ef4444">
                    <X size={12}/> Effacer
                  </Btn>
                )}
              </div>
            ...
            <motion.div key="rankings" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
                ...
                {rankings.length > 0 && (
                  ...
                  >
                    <Printer size={16} /> Imprimer le classement
                  </motion.button>
                )}
              </div>
